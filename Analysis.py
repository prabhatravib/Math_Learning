...
from token_visualization import (
...
def analyze_multiple_pdfs(pdf_files: List[str], fraction: float = 1.0) -> Dict[str, Dict[str, Any]]:
    """
    Analyze multiple PDF files and return combined results
    
    Args:
        pdf_files: List of PDF file paths to analyze
        fraction: Fraction of each PDF to process (0.0 to 1.0)
        
    Returns:
        Dictionary containing results for each PDF
    """
    all_results = {}
    ...
    for pdf_file in pdf_files:
        ...
        try:
            results = process_pdf(pdf_file, fraction)
            all_results[pdf_file] = results
            
            # Print summary for this file
            print(f"\nSummary for {os.path.basename(pdf_file)}:")
            print(f"Total tokens: {results.get('total_tokens', 0)}")
            print(f"Mathematical: {len(results.get('mathematical', []))}")
            print(f"Software: {len(results.get('software', []))}")
            print(f"Machine-related: {len(results.get('machine_related', []))}")
            print(f"Stop words: {len(results.get('stop_words', []))}")
            print(f"Numbers/Characters: {len(results.get('numbers and characters', []))}")
...
def main(pdf_paths=None, fraction: float = 1.0, create_chart: bool = True) -> Dict[str, Dict[str, Any]]:
    """
    Main function to process PDF files and optionally create visualizations.
    
    Args:
        pdf_paths: List of PDF file paths or single path string
        fraction: Fraction of each PDF to process
        create_chart: Whether to create visualization chart
        
    Returns:
        Dictionary with categorized tokens for all PDFs
    """
    # Default PDF paths if none provided
    if pdf_paths is None:
        pdf_paths = [
            r"C:\Users\prabh\Downloads\A Few Useful Things to Know About Machine Learning.pdf",
            r"C:\Users\prabh\Downloads\Bishop-Pattern-Recognition-and-Machine-Learning-2006.pdf"
        ]
    ...
    # Check OpenAI API connection first
    try:
        setup_openai_api()
    ...
    logger.info(f"Analyzing {len(pdf_paths)} PDF file(s)...")
    all_results = analyze_multiple_pdfs(pdf_paths, fraction)
    
    # Save results to JSON
    save_results_to_json(all_results, 'token_analysis_results.json')
    
    # Create visualization if requested
    if create_chart:
        logger.info("Creating comparison chart...")
        create_token_comparison_chart(all_results, 'token_comparison_chart.png')
    
    # Print overall summary
    print(f"\n{'='*50}")
    print(f"OVERALL ANALYSIS SUMMARY")
    print(f"{'='*50}")
    print(f"PDFs analyzed: {len(pdf_paths)}")
    print(f"Analysis fraction: {fraction*100:.1f}%")
    
    for pdf_file, results in all_results.items():
        if "error" not in results:
            total = results.get('total_tokens', 0)
            math_pct = (len(results.get('mathematical', [])) / total * 100) if total > 0 else 0
            machine_pct = (len(results.get('machine_related', [])) / total * 100) if total > 0 else 0
            
            print(f"\n{os.path.basename(pdf_file)}:")
            print(f"  Mathematical tokens: {math_pct:.1f}%")
            print(f"  Machine-related tokens: {machine_pct:.1f}%")
    
    if create_chart:
        print(f"\nVisualization saved as 'token_comparison_chart.png'")
    print(f"Detailed results saved as 'token_analysis_results.json'")
...
if __name__ == "__main__":
    ...
    # Check if PDF paths are provided as command line arguments
    if len(sys.argv) > 1:
    ...
    # Process all tokens from the PDFs (set fraction to 1.0 for 100%)
    fraction = 1.0  # Process 100% of each PDF
    
    # Run the analysis and create visualizations
    results = main(pdf_paths, fraction, create_chart=True)
